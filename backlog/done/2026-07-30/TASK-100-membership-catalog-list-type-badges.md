...
## Acceptance criteria
- [x] Строки `SingleVisit`, `Term` и `Professional` не содержат никаких badges.
- [x] Если название варианта равно `Профессиональный`, оно отображается один раз как название и не дублируется меткой.
- [x] Нулевое количество badges для всех трёх behavior kinds закреплено
  component test.
- [x] Название, цена, период доступности и edit action сохранены.
- [x] Длинное название переносится без clipping и horizontal scroll, а действие
  `Изменить` остаётся видимым и доступным без пересмотра placement из TASK-093.
- [x] Create/edit forms и backend behavior contracts не изменены.
- [x] На 390 x 844, 420 x 912, 440 x 956, 768 x 1024 и 1440 x 1200 строка не получает horizontal scroll или clipping.

## Test checklist
- [x] Добавить list-row cases для `SingleVisit`, `Term` и `Professional`.
- [x] Проверить нулевое количество любых badges внутри каждой строки.
- [x] Проверить, что create form по-прежнему содержит выбор поведения для разрешённых типов.
- [x] Добавить отдельный membership-catalog Playwright spec и populated-catalog
  сценарий в target iPhone spec.
- [x] При реализации запустить frontend unit tests, lint и build.
...
- implementation_plan: /backlog/done/2026-07-30/TASK-100-membership-catalog-list-type-badges.plan.md
- implementation_branch: fix/TASK-100-membership-catalog-list-type-badges
- implementation_state: completed
- implementation_commit: aad79ac
...
- Related completed task: `backlog/done/2026-07-19/TASK-070-membership-catalog.md`.
