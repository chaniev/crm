...
- implementation_plan: /backlog/done/2026-07-30/TASK-099-audit-log-remove-action-column.plan.md
- implementation_branch: fix/TASK-099-audit-log-remove-action-column
- implementation_state: completed
- implementation_commit: e2c75f9
...
## Acceptance criteria
- [x] В header журнала нет колонки `Действие`.
- [x] В каждой строке desktop и mobile нет отдельной cell/label/value `Действие`.
- [x] Дата/время, описание, пользователь и кнопка `Детали` сохранены.
- [x] Фильтр по типу действия продолжает работать.
- [x] Details modal по-прежнему показывает диагностическую информацию, включая action type, если она была частью текущего контракта.
- [x] Grid/table semantics соответствуют фактическому числу колонок.
- [x] На 390 x 844, 420 x 912, 440 x 956, 768 x 1024 и 1440 x 1200 нет horizontal page scroll или пустой колонки.

## Test checklist
- [x] Расширить `AuditLogScreen` component test: отсутствует header/cell `Действие`, остальные данные и details action видимы.
- [x] Проверить action-type filter после удаления display column.
- [x] Проверить details modal и focus return.
- [x] Обновить affected audit Playwright geometry на mobile/tablet/desktop.
- [x] Запустить `cd frontend && npm run test:unit`.
- [x] Запустить `cd frontend && npm run lint`.
- [x] Запустить `cd frontend && npm run build`.
- [x] Запустить affected Playwright checks.
...
## Completion notes
- Audit list сокращён до четырёх semantic columns; orphan grid areas удалены,
  а освободившаяся ширина передана описанию.
- `actionType` сохранён в фильтре, API/types и details modal.
