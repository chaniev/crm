...
- `ClientMembership.PaymentAmount` остается валовой суммой продажи;
- несколько частичных возвратов по одной продаже разрешены, но суммарный возврат не должен превышать валовую сумму продажи;
- для отчетов должен быть stable sale identity или явная backend reporting projection;
...
- Использовать sale identity/backend reporting projection из `TASK-036`; не строить отчет напрямую поверх всех строк `ClientMemberships`.
- Считать продажи по purchase date из финансовых продаж: `NewPurchase` и `Renewal`.
...
- Считать gross sales как сумму исходных `PaymentAmount` продаж без уменьшения на возвраты.
- Считать refund total как сумму `ClientMembershipRefund.Amount`.
...
- Создание модели возвратов, sale identity или membership refund summary, если они еще не выполнены в `TASK-036`.
...
- [ ] Продажи считаются по дате покупки через sale identity/reporting projection из `TASK-036`.
...
Не требуется перед планированием. Перед реализацией остановиться, если `TASK-036` не предоставил stable sale identity/reporting projection или если product decision потребует исторический snapshot филиала/группы/тренера на момент покупки вместо текущих backend-связей.
