...
- Обеспечить, что активная привязка клиента к филиалу единственная на дату, а исторические периоды не пересекаются.
...
- Клиент не может иметь две активные филиальные привязки на одну дату.
...
- [ ] Проверить периодные связи: клиент-филиал без пересечений, клиент-группа с несколькими группами, тренер-группа с несколькими группами.
- [ ] Проверить чистую миграцию/schema setup: `ClientMembershipSale`, required `SaleId`, refund cancellation fields, attribution period tables, FK/indexes и seed/test fixtures.
