...
- Чтение карточки клиента защищено `ViewClients` и существующим backend scope для тренера. Изменение membership защищается `ManageClients`; этот же policy должен защищать изменение комментария, что соответствует ролям Administrator/HeadCoach без новой permission/RBAC-модели.
...
- Комментарий нормализуется backend-ом: trim, whitespace-only → `null`, максимальная длина задаётся доменным контрактом (предлагается 1000 символов, согласованно с другими membership comments). Сравнение выполняется после нормализации; no-op не меняет metadata и не создаёт audit event.
- При `null -> text` и `text -> changed text` сохраняются actor и одно серверное UTC-время; при `text -> null` комментарий и оба metadata поля очищаются. Metadata состоит из nullable `CommentChangedByUserId` и `CommentChangedAt`.
...
- Карточка клиента уже показывает текущий абонемент и таблицу версий. UI должен группировать/показывать комментарий по `saleId`, чтобы несколько технических версий одной покупки не выглядели как разные комментарии. Минимальный вариант — компактный comment block для выбранной purchase/sale с действием редактирования у пользователей, которым backend разрешает mutation; frontend не определяет доменные права самостоятельно.
...
4. **До production-кода** добавить backend integration tests в `ClientsApiTests` или отдельный focused test class:
   - Administrator и HeadCoach могут установить/изменить/очистить comment;
   - Coach и unauthenticated request получают штатный forbid/unauthorized, запись и audit не меняются;
...
6. **До production-кода** добавить frontend component tests:
   - comment и `Имя · локальная дата, HH:mm` отображаются для выбранной purchase/sale;
   ...
   - legacy/empty state не фабрикует автора или `Invalid Date`;
...
10. Реализовать application command/result в `IClientMembershipService` и `ClientMembershipService`:
   ...
   - change/clear обновляют только три comment fields и возвращают transition для endpoint audit;
   - существующие purchase/correct/payment/refund/write-off методы не должны копировать или изменять comment metadata.
11. Расширить membership load/projection paths (`ClientMembershipSnapshotResult`, `ClientMembershipResponse`, `MapMembership`) данными sale comment. Загрузить comment author одной projection/include без N+1. Для всех versions одной sale возвращать согласованные значения; полную пару name/time выдавать только при разрешимом авторе.
...
14. Согласовать failure semantics с существующими membership mutations до написания audit error handling. Предпочтительно: comment persistence остаётся успешной, а audit write выполняется через локализованный best-effort helper с безопасным structured log, как в TASK-068. Не расширять задачу до outbox/общего redesign; если policy проекта требует атомарный audit для membership mutations, остановиться и вынести это решение на review.
...
16. Реализовать компактный UI в `ClientMembershipSection`: выбрать конкретную purchase/sale, показать comment и attribution, дать Administrator/HeadCoach action редактирования без дублирования на каждой технической версии. Переиспользовать Mantine/Onest и существующие form/notification patterns; проверить длинный текст/имя и narrow viewport.
...
- New writes keep comment/author/time consistent; clear sets all three to `null`.
...
- Existing coach client visibility remains backend-scoped. Comment mutation remains under `ManageClients` for Administrator/HeadCoach.
...
- Authorization matrix: Administrator, HeadCoach, Coach, unauthenticated.
...
- [ ] Administrator/HeadCoach succeed; Coach/anonymous and cross-client sale lookup cannot mutate data.
- [ ] Set/change/clear update exact metadata; normalized no-op preserves metadata and emits no audit.
...
- Best-effort audit failure after persistence creates a partial-success state. Reuse a localized safe pattern only after confirming membership audit policy; do not silently introduce inconsistent semantics across membership actions.
...
- endpoint невозможно авторизовать существующим `ManageClients` без RBAC redesign;
- существующий audit policy требует атомарности, которую нельзя обеспечить локально без outbox/system-wide redesign;
