...
1. Первый ряд: `Отчет: {period}` и conditional reset action. Period берется
   из displayed backend `report.period`, когда successful report существует.
...
3. Active-filter count/reset affordance, когда applied filters отличаются от
   cleared baseline. Count считается по трем пользовательским категориям:
...
- selected branch id exists but options are loading: `Филиал загружается`.
- selected branch id exists but option failed/missing: `Выбранный филиал`;
  adjacent text or accessible label must say `Название филиала не загрузилось`.
- `trainerId === null`: `Все тренеры`.
- found trainer option: current option label, preserving `fullName (login)`.
- selected trainer id exists but options are loading: `Тренер загружается`.
- selected trainer id exists but option failed/missing: `Выбранный тренер`;
  adjacent text or accessible label must say `Имя тренера не загрузилось`.
...
- button label should include active status, e.g. `Фильтры` plus active count
  ...
  that can overflow;
- refresh action remains visible as the only toolbar action beside filters;
...
- keep immediate-apply semantics. `Готово` only closes the drawer; it must not
  imply delayed apply.
- `Сбросить` resets only current finance report filters to existing
  `createClearedFilterValues()` behavior and leaves backend interpretation
  unchanged.
...
- active if `periodPreset`, applicable date fields, `branchId` or `trainerId`
  differ from the existing cleared baseline.
- reset must clear branch/trainer and return to the current reset behavior
  (`month` with empty date fields), then issue the same API query shape produced
  today by `toFinancialReportParams`.
...
Retry placement:
- error without data: inside report surface `ErrorState`, action `Повторить`.
- stale error with data: inside the same report surface alert, action
...
- fifth item always spans both columns at `360-440px`, чтобы layout был
  deterministic и не зависел от измерения текста;
...
- if `isZeroReport(report)` is true, show one explicit empty report state
  before any zero metrics.
- preferred implementation: do not render the KPI strip for the empty state.
  If product later requires zero totals to remain visible, use a single compact
  totals line inside the empty state, not five separate zero cards.
...
- actions: `Сбросить фильтры` only when active filters exist; no action when
  already at cleared scope.
...
- Error without data: report surface `ErrorState`, no KPI/breakdowns, retry
  inside the surface.
- Stale error with data: keep previous report, mark surface as stale with text
  and status semantics, include retry inside the report surface, and do not
  relabel previous money with failed filters.
- Success empty: one empty state, no repeated zero KPI cards and no empty
  breakdown triplet.
- Success non-empty: KPI strip followed by breakdown.
- Options error: warning stays near filters; report can still render. Missing
  labels use explicit fallback copy instead of guessed names.
...
- visible loading/refreshing feedback should appear in the same render cycle as
  `reportLoading`;
- stale alert appears after failed request and remains until a successful retry
  or another successful filter request.

## Focus, keyboard and temporary surfaces
- Focus order on mobile: scope summary/reset -> filter trigger -> refresh ->
  report status/retry -> KPI items if focusable none -> breakdown controls.
...
- Derive scope labels from loaded options and selected ids; never infer
  unauthorized branch/trainer names.
...
- Shared `CompactFilterPanel` changes are avoidable. Modify it only if local
  finance composition cannot meet active/reset/a11y requirements without
  duplicating a shared bug fix.
...
2. Model one last-successful `{ report, filters, scopeLabels }` snapshot beside
   current requested filters so stale data cannot be relabeled.
3. Prefer finance-local components/state over a shared abstraction; change
   `CompactFilterPanel` only for a demonstrated reusable accessibility gap and
...
- possibly `frontend/src/features/shared/ux.test.tsx` only if a small,
  justified `CompactFilterPanel` API/accessibility extension is required
...
5. Before production code, add/update component tests for:
   - `FinanceReportPresentation.test.tsx`: pure scope fallbacks, active count,
   ...
   - retry repeats the failed applied params; a later success replaces the
     snapshot and clears stale semantics;
   - refreshing with previous data is announced as pending, not success,
     empty or stale;
   - branch/trainer label fallbacks for loading, missing and options-error
     states;
   - reset clears only finance filters and preserves existing query semantics.
...
9. Add local display-state model for last successful report/scope and requested
   scope; update it only on successful `getFinancialReport`.
10. Add finance-local `FinanceReportPresentation.tsx` with
    `FinanceScopeHeader` and active/reset semantics using existing filter
    values, branch/trainer options and backend `report.period`.
...
15. Preserve existing `toFinancialReportParams`, `updateFilters`,
    `handleResetFilters`, permissions and API calls.
...
- [ ] Red component tests for non-empty, zero/empty, loading, error without
      data, stale-data error and label fallback.
...
- Financial trust risk: compact KPI styling can visually overemphasize or hide
  a value; human review must approve the final hierarchy and emphasis.
- Label trust risk: branch/trainer option load failure can tempt guessing
  labels; use explicit fallback copy instead.
...
- Shared-component risk: modifying `CompactFilterPanel` globally could affect
  audit/schedule/client screens; prefer local composition or add shared tests.

## Human review required before active implementation
- Approve that empty reports suppress the five zero KPI surfaces and show a
  single empty state.
- Approve final KPI visual emphasis, especially whether all five metrics have
  equal weight or `Чистая выручка` is highlighted.
- Approve stale copy wording for financial trust, including whether to show
  both displayed scope and failed requested scope.
- Approve branch/trainer missing-label fallback copy because it affects trust
  in access-scoped financial data.
- Confirm that no export/details/sales drill-down controls are required for
  this task.
...
- selected branch/trainer names would have to be guessed from unauthorized or
  unavailable data;
- active implementation is requested before human review approves the financial
  trust decisions above;
...
`Safe for Codex: no`, and financial-trust decisions require human review before
moving the task into active `/backlog/implementation`.
