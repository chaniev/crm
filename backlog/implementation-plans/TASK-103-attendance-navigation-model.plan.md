...
- не включать TASK-104, TASK-111 или другой redesign attendance/Home/schedule;
...
canonical route `/attendance`. `Главная` остаётся management inbox
`Требуют внимания`, `Расписание` остаётся отдельным `/schedule`, а route,
...
- Frontend `AppSection`, session mapper и route registry не знают
  `Attendance`; `/attendance` намеренно закреплён unit-тестом как `not-found`.
...
- Mobile navigation сейчас резервирует `Home`, `Schedule`, `Clients` и
  ...
  active overflow promotion должен вытеснять последний non-mandatory item, но
  никогда не вытеснять `Attendance`.
...
| Administrator | `Attendance` | `Home`, `Attendance`, `Schedule`, `Clients`, `Groups`, `Audit`, `Settings` |
| HeadCoach | `Home` | `Home`, `Attendance`, `Schedule`, `Clients`, `Groups`, `Users`, `Audit`, `Finance`, `Settings` |
| SuperAdministrator | `Home` | `Home`, `Attendance`, `Schedule`, `Clients`, `Groups`, `Users`, `Audit`, `Settings` |
...
- Coach does not receive or display `Home`;
- Administrator, HeadCoach and SuperAdministrator retain `Home` as management
  inbox and receive `Attendance` as a separate direct navigation item.
...
- Administrator primary path is the same; `Главная` remains separately
  reachable as management inbox.
- HeadCoach/SuperAdministrator primary landing: login -> `/` -> active
  `Главная`; `Посещения` is reachable with one direct navigation action.
- Direct `/` for Coach remains at the requested URL and renders the TASK-088
  restricted state; primary recovery is `Открыть Посещения` to `/attendance`.
...
Hierarchy:
1. `AuthenticatedShell` / existing `AppLayout`.
2. `RouteViewport` for section `Home`.
3. `HomeDashboard` with `PageLayout showHeader={false} title="Главная"`.
4. `AttentionPanel` only.

Rules:
- remove `Tabs`, attendance tab/panel, `AttendanceWorkspace`, attendance return
  context and `canMarkAttendance` branching from `HomeDashboard`;
- keep visually-hidden `h1` `Главная`, named `main` `Главная` and document
  title `Главная • {clubName}`;
- retain `Требуют внимания` as the visible/semantic management-inbox operation
  label without introducing a decorative card, statistics widget or helper
  paragraph;
- keep `AttentionPanel` loading, stale-success/error, empty, retry, row actions
  and client opening behavior unchanged.

### Direct restricted `/` for Coach
- do not call `replaceState` on initial denial;
- visible focused `h1`: `Нет доступа`;
- denial describes inaccessible `Главная`;
- primary recovery action: `Открыть Посещения` -> `/attendance`;
- named `main`: `Нет доступа`;
- document title: `Главная — нет доступа • {clubName}`;
...
- desktop order: `Home`, `Attendance`, `Schedule`, `Clients`, `Groups`,
...
- `Attendance` is mandatory primary mobile navigation whenever authorized;
- Coach stable primary: `Посещения`, `Расписание`, `Клиенты`;
- management stable primary: `Главная`, `Посещения`, `Расписание`, `Клиенты`;
- if an authorized overflow destination is active, promote it by replacing the
  last non-mandatory stable item (normally `Клиенты`); move the displaced item
  into overflow and never displace `Посещения`;
...
- `768 x 1024` and `1440 x 1200`: sidebar is visible, bottom navigation hidden,
  `Посещения` remains a direct item, and removed Home/attendance duplication is
...
5. `react-specialist` consumes the approved contracts and implements the
   frontend mapper/route/navigation/Home/return changes after frontend red
...
6. Update backend HTTP integration tests in `AuthorizationFlowTests.cs` with the
   ...
   assertions to prove representation changed without privilege expansion.
7. Update frontend API mapper unit tests so session JSON containing
   `Attendance` maps it in both `landingScreen` and `allowedSections`, while
   unknown values still fail closed under the existing mapper policy.
8. Update `appRoutes` unit tests before implementation:
   - `/attendance` parse/serialize, exact label/path and allowed resolution;
   - Coach `/` restricted outcome with `/attendance`/`Посещения` recovery;
   - all role navigation inventories and exact desktop ordering;
   - stable mobile primary sets and active overflow promotion which never
     displaces `Attendance`;
...
9. Update shared navigation/component tests before implementation:
   ...
   - named `main` landmark for allowed Attendance, Home and restricted state.
10. Update Home/Attendance component tests before implementation:
    - Home renders management inbox only, without tablist or attendance API
      calls, while attention loading/error/empty/success remain intact;
...
11. Update client-profile return-state unit tests before implementation:
    ...
    - bump serialization to version 2; safely normalize legacy version-1
      `groupEdit` context because its route is unchanged, but reject legacy
      version-1 attendance/`Home` context and use the existing Clients fallback
      because its history entry cannot satisfy the new canonical URL.
12. Update `App` integration tests before implementation:
    ...
    - direct Coach `/` denial, recovery and no initial silent redirect;
...
13. Update Playwright tests before implementation:
    - Coach and Administrator login land on `/attendance` with active
      `Посещения`; Coach has no `Главная`;
    - HeadCoach/SuperAdministrator land on `/`, see management inbox only and
      reach attendance in one direct action;
...
15. Run the focused backend authorization tests. Expected red reason: current
    `AccessScopeService` still returns `Home` landing for Coach/Administrator
    and omits `Attendance` from every role.
16. Run the focused frontend unit/component tests. Expected red reasons:
    mapper drops `Attendance`, `/attendance` is `not-found`, mobile ordering
    lacks the mandatory section, Home still owns attendance and return origin
    is still `Home`.
...
19. Update `AccessScopeService` with the exact role matrix. Reuse the existing
...
22. Add `Attendance` to the typed API section union and session mapper.
23. Add exact `Посещения` label, `/attendance` path, desktop inventory and
    route/access resolution in `appRoutes`.
24. Adjust mobile navigation calculation around an explicit mandatory
    `Attendance` item plus non-mandatory candidates; preserve the existing
    four-item limit and adaptive current-destination promotion.
25. Add the attendance icon through the existing exhaustive section icon
    factory and keep all section switches exhaustive.
...
28. Reduce `HomeDashboard` to management `AttentionPanel`; remove attendance
    tab state/mounting and dead imports/props without changing inbox behavior.
29. Preserve a semantic/visible `Требуют внимания` operation label after tabs
    are removed; do not add unrelated Home redesign.
30. Change attendance client origin and origin-route checks to section
...
31. Verify browser history composition still preserves client-list and
...
32. Mechanically align affected test session fixtures and old-model assertions;
    do not change unrelated product expectations.

### Phase 6 — green and regression validation
33. Rerun all new focused tests green, then run the complete backend and
    frontend unit suites, lint and production build.
34. Run affected Chromium Playwright specs for attendance, auth landing, Home,
    route access, client-profile return, responsive screens and touch/navigation
    inventory.
35. Run target-iPhone WebKit projects and compact-height coverage. Treat desktop
    Chromium viewport resizing as geometry evidence, not iPhone Safari proof.
36. Start a task-local Docker Compose stack only after automated green checks:
...
37. On the integrated backend/frontend stand, smoke all four role landings,
    actual session JSON, direct `/attendance`, Coach direct `/` recovery,
    management Home and attendance client return. Stop the exact task stack
    without deleting volumes unless the implementation workflow needs it kept.
38. Review final diff for product-code scope, generated artifacts, fixture drift
...
4. No feature flag is required for a coordinated CRM release. Backend-first or
   ...
   `Attendance` and new frontend cannot serve old Coach `Home`; deploy/rollback
   the paired backend and frontend artifacts together.
...
- `backend/src/GymCrm.Application/Authorization/AppSection.cs` only if discovery
  shows the existing `Attendance` constant differs; no change currently expected
...
- `frontend/src/features/home/AttentionPanel.tsx` only for the retained
  `Требуют внимания` operation label if it is not owned by `HomeDashboard`
...
### Unit tests
- session mapping of `Attendance` and unknown-section fail-closed behavior;
...
- Home/Attendance render ownership, headings and no duplicate workbench mount;
...
- HeadCoach/SuperAdministrator `/` management inbox and direct attendance entry;
- Coach restricted `/` with exact recovery;
...
- frontend route/mapper/history/component tests fail on missing Attendance
  contract and old Home ownership;
...
Secondary barrier: `appRoutes`, session mapper, `App`, Home/Attendance and
return-context unit/component tests lock canonical `/attendance`, separate Home,
typed recovery, main/title/nav semantics, mandatory mobile placement and safe
client return.
...
- Mobile four-slot promotion can accidentally move `Attendance` to overflow or
  mark `Ещё` current; exact unit and target-device tests are mandatory.
...
- Removing Home tabs can accidentally remove management loading/error/empty or
  client actions; component and Home E2E regressions must remain.
...
- Before deployment verify health plus four role session contract and both `/`
  and `/attendance` in the isolated stand.
...
- product role matrix or `Home` management-inbox decision differs from the
  source task;
