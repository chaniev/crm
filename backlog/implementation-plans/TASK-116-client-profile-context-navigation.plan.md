...
- Задача medium risk, `Safe for Codex: yes`, локализуема во frontend и имеет
  реалистичный automated regression barrier. Critical clarification questions
  отсутствуют.
...
- Browser Back/Forward использует те же history entries и sanitized typed
  context. Malformed, stale или incompatible state fail-closed в существующий
  `/clients` fallback и не позволяет произвольный return path.
...
- Перед push в client details сохранить тот же context в текущем origin entry
  ...
  explicit back может использовать verified `returnDepth`.
- Context сохраняется через scoped `clientDetails -> clientEdit ->
  ...
  чтобы редактирование из карточки не ломало TASK-116 origin. На unrelated
  route context удаляется.
...
- Attendance snapshot применяется только после загрузки разрешённых групп:
  не отправлять roster request для невалидированного stale `groupId`. Если
  group/date больше недоступны, применить existing first-group/today fallback,
  показать доступное recovery message и не выводить forbidden option.
...
- Native button activation поддерживает Enter/Space. Browser Back/Forward и
  explicit back имеют одинаковый restored context; positive tab indices и
  breakpoint-specific duplicate DOM запрещены.
...
6. До production-кода расширить `AttendanceScreen.test.tsx` и
   ...
   - stale group/date fallback доступно объяснён и не вызывает forbidden
...
7. До production-кода создать `GroupClientRow` component tests и расширить
   ...
   - `GroupEditScreen` передаёт current `groupId`/origin callback;
...
8. До production-кода расширить `App.test.tsx` как route-integration barrier:
   ...
   - client details -> edit -> details -> back не теряет origin;
...
9. До production-кода добавить focused
   ...
   - group edit: открыть client после status badge и вернуться на exact
     `/groups/{groupId}/edit`;
...
10. До production-кода расширить responsive coverage:
    ...
    - action >= `44 x 44px`, independent controls separated >= `8px`, long
      names/statuses wrap, no overlap, body/document no horizontal overflow;
...
12. Реализовать versioned client-profile return-state helper с sanitized union,
    merge/read/strip, scoped route retention и bounded depth. Переиспользовать
    `AppRoute`/`getRoutePath`; не добавлять альтернативный path parser.
...
14. Передать context props через `RouteViewport`, `HomeDashboard`,
    `AttendanceWorkspace` и `GroupEditScreen`. Сохранить snapshot в origin
    entry перед push; восстановить attendance values только после allowed-group
    reconciliation.
...
17. Выделить/добавить `GroupClientRow` и передать exact open callback из
    `GroupEditScreen`. Сохранить read-only content и existing empty/error form
    behavior.
...
5. Local Mantine action + CSS, затем focused Playwright и target-iPhone closure.
...
- Child rows передают `clientId` и typed origin в centralized callback; URL и
  permission decisions не дублируются вне routing boundary.
...
- Attendance group/date/view snapshot не применяется до сверки с backend-
  returned allowed groups/date boundaries.
...
- Сохранение несохранённого draft формы group edit; TASK возвращает exact
  route и server-loaded group context. Если продукт требует draft preservation
  или unsaved-changes guard, это отдельное решение и stop condition.
...
- `GroupClientRow`/`GroupEditScreen`: content, status-then-action order, exact
  group/client origin, empty/error and focus fallback.
...
### UI/e2e tests
- Attendance -> client -> back with exact group/date/view; group edit -> client
  -> exact group route.
...
- [ ] Native Back/Forward и details-edit-details сохраняют typed origin;
      malformed/direct context использует safe fallback.
...
- Group edit draft не сохраняется в scope. Если это становится обязательным
  product expectation, локальная navigation task превращается в отдельный
  dirty-form workflow.
...
- требуется сохранять unsaved group form draft либо вводить application-wide
  navigation blocker/global router;
