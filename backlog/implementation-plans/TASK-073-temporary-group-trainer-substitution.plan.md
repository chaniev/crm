...
- Управление группами защищено существующей policy `ManageGroups`, доступной `HeadCoach` и `Administrator`; обычному `Coach` отдельное локальное role-условие добавлять не требуется.
...
- Если замещающий позднее стал основным тренером, запись не отменяется автоматически: effective scope является union оснований, а окончание/cancel временного основания не отзывает постоянное.
- Деактивация пользователя блокирует его аутентификацию существующими правилами. Историческая запись и audit сохраняются.
- Деактивация группы запрещает новые/обычные edits замещения, но cancel остаётся доступным. Существующие group lifecycle/access rules не перерабатываются в этой задаче.
...
- `HeadCoach` сохраняет текущий elevated scope, `Administrator` не получает attendance access, а `Coach` получает только effective group ids.
...
- `GET /groups/{groupId}/trainer-substitutions` возвращает backend-sorted collection, backend-owned `status`, `allowedActions.canEdit/canCancel`, collection capability `canCreate` и nullable `createUnavailableReason`.
...
- Все routes наследуют `ManageGroups`; write routes также сохраняют существующую CSRF-защиту.
...
4. **До production-кода** добавить management integration tests в `GroupsApiTests` или отдельный focused test file:
   - HeadCoach и Administrator могут list/create/update/cancel; Coach получает `403`; anonymous — текущий auth result; write без CSRF отклоняется;
   ...
   - response status/actions/canCreate вычислены backend на фиксированной business date;
   ...
   - cancel сохраняет строку, немедленно исключает её из effective scope и повторный cancel не создаёт второй event.
5. **До production-кода** добавить fixed-date backend access integration tests:
   ...
   - `AttendanceApiTests`: groups, roster и save доступны на активном периоде и запрещены вне него; основной trainer не теряет доступ;
...
10. Реализовать минимальную domain/persistence модель:
    ...
    - обновить текущее initial database state, designer и snapshot, если repository policy к моменту реализации не изменилась; не добавлять вторую историческую migration без проверки этой policy;
...
14. Перевести `AccessScopeService` на effective service и затем заменить прямые security-sensitive проверки `GroupTrainers` в `ClientEndpoints`, `AttendanceEndpoints`, `ClientPhotoService` и `BotApiService`:
    ...
    - не менять role permissions и elevated HeadCoach/Administrator behavior;
...
15. Реализовать frontend typed API в отдельном `groupTrainerSubstitutions.ts` и независимо загружаемую `GroupTrainerSubstitutionsSection` между основной формой edit screen и списком клиентов:
    ...
    - Active/Upcoming показывать сразу, Expired/Cancelled — в доступной раскрываемой истории, используя backend order/status;
...
Feature flag не требуется для локального early-stage rollout при атомарной поставке backend/frontend и воспроизводимой schema setup. Если слои должны выкатываться раздельно, сначала допустима additive schema/backend deployment; frontend остаётся совместимым, пока новый endpoint не используется.
...
- Keep response/queries bounded and avoid N+1; effective ids are loaded once per request/use case.
...
- Frontend API mapping preserves backend status/order/actions.
...
- Full management authorization + CSRF matrix.
...
- HeadCoach/Administrator elevated behavior and Coach permissions remain unchanged.
...
- [ ] HeadCoach/Administrator management succeeds; Coach/anonymous/CSRF-negative cases fail correctly.
...
- [ ] Frontend renders backend status/actions, handles mutation errors and keeps permanent trainer ids unchanged.
...
Причина: задача остаётся high-risk (`Safe for Codex: no`) и меняет security-sensitive effective access сразу в нескольких consumers. План локализует модель, контракт, тестовую матрицу и regression barriers, но до реализации требуется явный risky-task review/перевод в implementation и подтверждение двух продуктовых решений: inclusive date boundaries и conflict semantics для одной пары group+substitute.
