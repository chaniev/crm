...
Source task remains `risky` and is not moved to `/backlog/implementation`: `Risk level: high`, `Safe for Codex: no`. This plan is created for explicit product/architecture review before implementation execution.
...
## Goal
Любое новое назначение, продление или создающий новую продажу перевод клиентского абонемента атомарно фиксирует состоявшуюся оплату. Пользователь выбирает только фактическую дату оплаты — сегодня либо допустимую прошедшую дату. Backend больше не хранит и не распространяет paid/unpaid state, а дата оплаты остаётся отдельной от даты оформления продажи, времени записи и периода действия абонемента.

## Proposed product and contract decisions for review

Эти решения локализуют high-risk scope и должны быть подтверждены при переводе задачи в implementation:
...
3. API может называть существующую атрибуцию sale явно: `paymentDate`, `paymentRecordedByUserId`, `paymentRecordedByUserName`, `paymentRecordedAt`. Последние три проецируются из `CreatedByUserId`, `CreatedByUser` и `CreatedAt`; отдельные дублирующие payment-actor/time колонки не добавляются.
...
6. Transitional compatibility применяется только на write boundary:
   - отсутствие legacy status либо `paymentStatus=Paid`/`isPaid=true` допускается на один переходный релиз;
   - `paymentStatus=Unpaid` или `isPaid=false` отклоняется до idempotency reservation и до любых записей стабильным `400 membership-payment-status-removed`;
...
8. Нижней границы по `PurchaseDate`, `IndividualValidFrom` или `IndividualValidTo` нет: backdated payment может предшествовать созданию записи и периоду действия. Она не меняет срок абонемента, attendance history или effective membership.
...
10. Web mark-payment endpoint и internal bot unpaid/mark-payment endpoints на один переходный релиз становятся authenticated tombstones с `410 membership-payment-action-removed`, не создают version/audit/idempotency records и затем могут быть удалены отдельной cleanup-задачей.
11. Из read contracts удаляются `isPaid` и `hasUnpaidCurrentMembership`. `hasActivePaidMembership` переименовывается в `hasActiveMembership`; `ActivePaid` переименовывается в `Active`; unpaid filters, states, warnings, attention reasons и debt lists удаляются.
...
14. Refund event остаётся в периоде `RefundDate`; правила refund validation, суммы и cancellation не меняются.
15. Purchase, renewal и transfer сохраняют sale, assignment/version, обязательный audit и idempotency completion в одной transaction. Ошибка validation/audit/constraint не оставляет частичных данных.
...
- TASK-070 и TASK-078 остаются историческими завершёнными задачами. TASK-083 заменяет только их paid/unpaid решение и не переоткрывает catalog, lifecycle, pricing, correction targeting или idempotency rules.
...
- Добавить unit/API/PostgreSQL tests, описывающие обязательную payment date, запрет future/unpaid payload, sale-owned metadata и отсутствие частичных записей.
- Зафиксировать red evidence на purchase, renewal и transfer до production changes.
- Review gate: подтвердить предложенную sale-owned модель, правила дат, отчётный период и отсутствие необходимости сохранять существующие production rows.
...
- Перевести purchase/renew/transfer на обязательную payment date и единый backend policy.
- Сохранить текущие lifecycle, permissions, pricing, optimistic target и idempotency barriers.
...
- Разделить accounting и attribution dates в financial report.
...
- Удалить status selectors/mark-payment/debt UI; сделать payment date обязательной и заполненной `client.businessDate`.
...
Slices являются последовательными фазами одной cross-layer задачи и должны быть реализованы в одной task branch, чтобы backend contract и оба consumers не разъехались. Если их позже превратить в независимые backlog TASKs, каждая получает отдельную branch и явную compatibility/deployment dependency.

## Execution steps
1. Получить явное разрешение на high-risk execution, убедиться, что source task перемещён в `/backlog/implementation`, проверить clean `main`, выполнить pull и создать `feature/TASK-083-membership-payment-status-simplification`.
...
3. Подтвердить runtime database policy: окружение пересоздаётся с нуля; production backfill/upgrade не требуется. Если это неверно, остановиться до schema edits.
4. **До production-кода** добавить unit tests для payment-date policy, legacy payload classification, sale-owned projection, status-free membership semantics и financial accounting/attribution date separation.
...
6. **До production-кода** добавить real-PostgreSQL integration/API tests для purchase, renewal, transfer, clean schema, idempotency, atomicity, audit, future/backdated dates, legacy payload и tombstone endpoints.
...
10. **До frontend production-кода** обновить/добавить API/component/Playwright tests: status selector и mark-payment отсутствуют, payment date обязательна, default/max равны `client.businessDate`, backdated value отправляется без изменения validity dates, backend ProblemDetails отображается.
...
15. Реализовать минимальный backend domain/application contract: required sale `PaymentDate`, без `IsPaid` и mark-payment mutation; добавить один small payment-date policy без HTTP/EF dependencies.
...
17. Обновить purchase/renew/transfer DTO, validation, commands, idempotency payloads и service creation flow. `Unpaid`/`false` legacy fields отклонять до reservation/writes; отсутствующий или paid legacy marker не управляет backend result.
18. Обеспечить одну transaction для sale, membership assignment/version, audit и idempotency completion во всех трёх write flows, включая transfer.
19. Удалить mark-payment service method/error/action semantics; заменить web/internal bot endpoints на approved authenticated `410` tombstones.
...
21. Обновить financial report: sale selection/first-sale ordering по `PaymentDate`; branch/group/trainer attribution по `PurchaseDate`; загружать assignment history по фактическому диапазону attribution dates событий, а не только по query period.
22. Обновить backend bot contracts/service и audit snapshots; payment date/recording actor/time должны одинаково читаться в current membership и каждой history version той же sale.
23. После backend contract green выполнить локальный `ui-designer` checkpoint и минимально обновить frontend: один required date field во всех создающих sale формах, default/max из `client.businessDate`, без paid badge/status selector/mark-payment/debt filter.
24. Обновить frontend types, endpoints, mappers, resources, home/attention/attendance/client-list consumers и confirmation/history rendering.
...
27. Пересоздать локальный PostgreSQL/stand из пустого состояния, проверить migrations/model drift, seed, health и smoke для today/backdated purchase, renewal, transfer, report, attendance, web reload и bot reads.
...
### Backend tests first
- `backend/tests/GymCrm.Tests/ClientMembershipPaymentSimplificationApiTests.cs` — новый focused purchase/renew/transfer/legacy/tombstone/audit/idempotency suite.
...
- Изменение membership validity, renewal duration, overlap, correction или single-visit lifecycle.
...
- Немедленное физическое удаление tombstone routes в том же релизе.
...
- Legacy `Paid`/`true` классифицируется как harmless compatibility marker; `Unpaid`/`false` даёт approved stable error; разные harmless формы имеют одинаковый semantic idempotency payload.
...
- Financial sale event имеет `AccountingDate=PaymentDate`, `AttributionDate=PurchaseDate`; refund сохраняет `RefundDate`.
...
- Transfer с ошибкой payment validation не меняет branch/groups/old membership; audit и все writes откатываются.
...
- Correction, write-off и restore не меняют sale payment metadata и не создают payment transitions.
- Mark-payment web/internal bot tombstones возвращают `410 membership-payment-action-removed` и ничего не пишут.
- Unpaid bot list tombstone возвращает approved `410`; active internal bot contracts не содержат unpaid fields/actions.
- Client list/details/attention не имеют unpaid filters/state/reasons/flags; supplied legacy unpaid filter получает documented stable rejection, а не silent wrong result.
...
- Financial report относит gross sale в `PaymentDate`, refund в `RefundDate`, first-sale count по первой payment date и branch/group/trainer по `PurchaseDate`.
...
### UI/e2e tests
- Today purchase, backdated purchase, renewal и transfer проходят через UI и reload.
...
## Test plan
- [ ] Unit red/green: payment date, legacy compatibility, status-free semantics, financial two-date projection.
- [ ] Real PostgreSQL red/green: purchase, renewal, transfer, atomicity, idempotency, audit, schema.
...
- [ ] Focused Playwright: today/backdated purchase, renewal, transfer, home/attendance and 320 px layout.
...
## Regression barrier
Основной barrier — real-PostgreSQL API suite, который на фиксированном backend business date создаёт today/backdated purchase, renewal и transfer, проверяет fresh reload, audit, idempotency и row cardinality, затем доказывает ноль writes для future/unpaid payload. Дополнительный financial test обязан показать sale в периоде `PaymentDate`, но branch/group/trainer attribution на `PurchaseDate`, включая backdated date до появления client assignment. Frontend component/Playwright и bot contract suites защищают consumers от возврата status selector, debt list или mark-payment action.

## Risks
- Перенос payment metadata с version на sale затрагивает schema, audit, history и большое количество fixtures.
- Backdated accounting date и purchase-date attribution могут изменить финансовые цифры и first-sale/new-client count в прошлых периодах.
- Если existing runtime database нельзя пересоздать, удаление колонок без migration/backfill небезопасно.
- Несогласованный deploy backend/frontend/bot может оставить consumers, ожидающие removed fields/endpoints.
...
- Transfer имеет сложную transaction boundary; audit после commit или вложенный service commit может оставить partial state.
...
- нет явного human approval high-risk execution или source task не переведён в implementation;
- worktree dirty, активная branch не соответствует TASK-083 либо не создана от актуального `main`;
- среда требует сохранить существующие unpaid production rows, но нет отдельного migration/backfill/rollback решения;
- product owner не подтверждает `PaymentDate` как accounting date или `PurchaseDate` как organizational attribution date;
...
Причина: подробный test-first план готов, но source task остаётся high-risk в `/backlog/risky` с `Safe for Codex: no`. Перед execution нужны явное одобрение предложенных product/contract decisions и подтверждение, что runtime database можно пересоздать без production data migration.
