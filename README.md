...
- Роли `HeadCoach`, `SuperAdministrator`, `Administrator`, `Coach` с разными доступными разделами, правами и областями доступа.
...
- `BOT_ENABLED` - включает Telegram polling; при `false` bot-owned storage и health
  endpoints остаются доступны, а подключение к Telegram не создаётся;
...
Скрипт применяет миграции, затем пересоздаёт только свои детерминированные
тестовые записи:

- 4 типа групп (существующие типы с такими именами переиспользуются), 4 филиала
  по 3 зала, 10 тренеров, 5 администраторов и 30 групп;
- 300 клиентов с детерминированными датами рождения и файлами фотографий;
- 150 годовых, 90 месячных и 15 профессиональных абонементов; 45 клиентов
  остаются без абонемента;
- 29 из 285 клиентов без профессионального абонемента состоят в двух группах,
  остальные — в одной;
- для каждой группы создаётся бессрочная серия расписания с даты запуска
  скрипта: одна группа занимается по воскресеньям в 10:30, остальные — по
  схеме пн–ср–пт или вт–чт–сб; расписания используют уже создаваемые seed-залы
  и не добавляют дополнительных помещений.

Пароль всех seed-пользователей: `1`. Администраторы распределяются по
seed-филиалам. Повторный запуск удаляет связанные seed-абонементы и расписания,
а затем воспроизводит тот же состав данных с новой датой запуска.
...
Для чистого стенда, где нужны главный тренер `headcoach`, суперадминистратор `sa`
и пять администраторов `admin1`–`admin5` филиала «Ленинский» без пользовательского
каталога абонементов, клиентов, групп, расписания и посещений:

```bash
./backend/scripts/seed-leninsky-admins-only.sh
```

Скрипт создаёт также единственный обязательный филиал, к которому привязаны
администраторы. Начальная миграция при этом сохраняет один системный элемент каталога;
сам seed элементы каталога не добавляет. Пароль всех семи seed-пользователей: `1`.
