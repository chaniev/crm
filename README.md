...
Скрипт применяет миграции, затем пересоздает только свои детерминированные тестовые записи: 4 типа групп (если типы с такими именами уже есть, они переиспользуются), 4 филиала по 3 зала, 10 тренеров, 5 администраторов, 30 групп и 300 клиентов с файлами фотографий. Пароль всех seed-пользователей: `1`.
Администраторы распределяются по seed-филиалам, а клиенты получают детерминированные даты рождения.
