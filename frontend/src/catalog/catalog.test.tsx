...
describe('development-only design-system catalog shell', () => {
  ...
  test('renders a smoke shell from production foundation registries', () => {
    ...
    expect(screen.getByRole('region', { name: 'Surface modes' })).toBeVisible()
    expect(screen.getByTestId('catalog-surface-list-row')).toHaveTextContent('без тени')
    expect(screen.getByTestId('catalog-surface-focus-card')).toHaveTextContent('с элевацией')
