...
При failed build/audit/test заблокируй remote mutations. Продолжай локальную
диагностику и исправление причин в пределах согласованной задачи, затем повтори
затронутые проверки. Не ослабляй проверки и не подавляй audit failures. Если
исправление требует изменения scope, контракта или явно закреплённого
пользователем target commit, согласуй необходимое решение; для уже разрешённых
изменений повторное разрешение не нужно. После локальных исправлений обнови
release evidence и собирай exact target из чистого checkout по правилам выше.

Останови remote mutations при неоднозначной migration, несовпадении
server identity/path, неожиданном dirty runtime state, failed backup/checksum,
непринятом поведении или конфликтующих выводах агентов. Продолжай безопасную
диагностику; возобновляй rollout только после устранения причины и повторного
go/no-go. При сбое уже начатого rollout следуй проверенному recovery ниже.
